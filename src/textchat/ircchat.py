...
class TextChat(App):
    ...
    MODES = {"irc": IRCScreen, "settings": SettingsScreen, "quit": QuitScreen}
    ...
    async def action_return_home(self) -> None:
        ...
        for channel in channels:
            try:
                self.SCREENS["irc"].query_one(TabbedContent).add_pane(
                    TabPane(
                        channel,
                        Label(),
                        name=channel,
            ...
            try:
                ...
                if now.minute <= 9 and event.value.split()[0] not in self.action_list:
                    self.irc_client.connection.privmsg(
                        self.tab.name.replace("@", "").replace("+", ""), event.value
                    )
                    self.tab.mount(
                        Label(
                            f"{now.hour}:0{now.minute} <{self.irc_client.nickname}> {event.value}"
                        )
                    )
                    ...
                    self.tab.mount(
                        Label(
                            f"{now.hour}:{now.minute} <{self.irc_client.nickname}> {event.value}"
                        )
                    )
    ...
    def irc_message(self, time, channel, sender, message, classes):
        self.tab = self.app.query_one(TabbedContent).get_pane(
    ...
    def received_private_message(self, time, sender, message, classes):
        try:
            ...
            self.tab = self.app.query_one(TabbedContent).add_pane(
                TabPane(sender, Label(), name=sender, id=f"{sender}")
            )
            self.tab = self.app.query_one(TabbedContent).get_pane(f"{sender}")
            self.tab.mount(Label(f"{time} <{sender}> {message}", classes=classes))
            self.notify(f"<{sender}> {message}", title="Private Message")
    ...
    def add_to_tree(self, channel, user_list):
        ...
        if (
            self.app.channel_list is not None
            and channel != self.app.channel_list.data["id"]
        ):
