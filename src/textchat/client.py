...
class IRCApp(SimpleIRCClient):
    ...
    def start(self):
        if self.sasl_login and self.server_port == 6697:
            ...
            wrapper = functools.partial(
                context.wrap_socket, server_hostname=self.server_name
            )
    ...
    async def _intercept_join(self, message):
        ...
        if message.startswith("/join"):
            ...
            if channel not in self.channels:
                ...
                self.app.query_one(TabbedContent).add_pane(
                    TabPane(
                        channel,
                        Label(),
                        name=channel,
    ...
    async def _intercept_part(self, message):
        ...
        if message.startswith("/part"):
            ...
            self.app.query_one(TabbedContent).remove_pane(
                f"{channel.replace('#', '').lower()}"
            )
    ...
    def on_pubmsg(self, connection, event):
        ...
        if now.minute <= 9:
            self.app.handle_irc_message(
                f"{now.hour}:0{now.minute}", channel, sender, message, classes=None
            )
        else:
            self.app.handle_irc_message(
                f"{now.hour}:{now.minute}", channel, f"{sender}", message, classes=None
            )
    ...
    def on_privmsg(self, connection, event):
        ...
        if now.minute <= 9:
            self.app.handle_private_message(
                f"{now.hour}:0{now.minute}", user, message, classes=None
            )
        else:
            self.app.handle_private_message(
                f"{now.hour}:{now.minute}", user, message, classes=None
            )
    ...
    def on_ctcp(self, connection, event):
        ...
        if now.minute <= 9:
            self.app.handle_irc_message(
                f"{now.hour}:0{now.minute}", channel, f"{sender}", message, classes
            )
        else:
            self.app.handle_irc_message(
                f"{now.hour}:{now.minute}", channel, f"{sender}", message, classes
            )
